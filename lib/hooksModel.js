...
module.exports = {
	beforeCreate: function(item, options) {
		...
		queryOptions.where[hierarchy.primaryKeyAttribute] = parentId;
	...
	afterCreate: function(item, options) {
		...
		var itemId = item[hierarchy.primaryKeyAttribute],
	...
	beforeUpdate: function(item, options) {
		...
		return Promise.try(function() {
			...
			where[hierarchy.throughForeignKey] = item[hierarchy.primaryKeyAttribute];
		...
		.then(function() {
			...
			var where = {};
			where[hierarchy.primaryKeyAttribute] = parentId;
		...
		.then(function(parentLevel) {
			...
			return Promise.try(function() {
				...
				sql = utils.replaceIdentifiers(sql, {
					...
					id: hierarchy.primaryKeyAttribute,
					itemId: hierarchy.throughKey,
					ancestorId: hierarchy.throughForeignKey
				}, sequelize);

				return sequelize.query(sql, null, {transaction: options.transaction, replacements: {id: item.id, levelChange: levelChange}});
			})
			.then(function() {
				...
				return sequelize.query(sql, null, {transaction: options.transaction, replacements: {id: item.id}});
			})
			.then(function() {
				...
				return sequelize.query(sql, null, {transaction: options.transaction, replacements: {id: item.id, parentId: parentId}});
			});
		})
		.return();
	},
