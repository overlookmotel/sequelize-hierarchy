...
module.exports = {
	beforeCreate: function(item, options) {
		...
		var queryOptions = {where: {}, attributes: [levelFieldName], transaction: options.transaction};
	...
	afterCreate: function(item, options) {
		...
		return Promise.try(function() {
			...
			var queryOptions = {where: {}, attributes: [hierarchy.throughForeignKey], transaction: options.transaction};
	...
	beforeUpdate: function(item, options) {
		...
		return Promise.try(function() {
			...
			return patches.find(hierarchy.through, {where: where, transaction: options.transaction})
		...
		.then(function() {
			...
			return patches.find(model, {where: where, attributes: [hierarchy.levelFieldName], transaction: options.transaction})
			.then(function(parent) {
		...
		.then(function(parentLevel) {
			...
			return Promise.try(function() {
				...
				return patches.query(sequelize, sql, {transaction: options.transaction, replacements: {id: item.id, levelChange: levelChange}});
			})
			.then(function() {
				...
				return patches.query(sequelize, sql, {transaction: options.transaction, replacements: {id: item.id}});
			})
			.then(function() {
				...
				return patches.query(sequelize, sql, {transaction: options.transaction, replacements: {id: item.id, parentId: parentId}});
			});
		})
		.return();
	},
