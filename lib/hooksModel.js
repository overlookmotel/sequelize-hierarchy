		...
		beforeUpdate: function(item, options) {
			...
			.then(function(parentLevel) {
				...
				return Promise.try(function() {
					...
					var schema = hierarchy.through.schema ? '*schema.' : '';
				...
				.then(function() {
					...
					if (['postgres', 'postgres-native'].indexOf(sequelize.options.dialect) != -1) {
						sql = 'DELETE FROM ' + schema + '*through ' +
							'USING ' + schema + '*through AS descendents, ' + schema + '*through AS ancestors ' +
							'WHERE descendents.*itemId = ' + schema + '*through.*itemId ' +
							'	AND ancestors.*ancestorId = ' + schema + '*through.*ancestorId ' +
						...
						sql = 'DELETE FROM ' + schema + '*through ' +
							'WHERE EXISTS (' +
							'	SELECT * ' +
							'	FROM ' + schema + '*through AS deleters ' +
							'		INNER JOIN ' + schema + '*through AS descendents ON descendents.*itemId = deleters.*itemId ' +
							'		INNER JOIN ' + schema + '*through AS ancestors ON ancestors.*ancestorId = deleters.*ancestorId ' +
							'	WHERE deleters.*itemId = ' + schema + '*through.*itemId ' +
							'		AND deleters.*ancestorId = ' + schema + '*through.*ancestorId ' +
							'		AND ancestors.*ancestorId = ' + schema + '*through.*ancestorId ' +
							'		AND ancestors.*itemId = :id ' +
							'		AND (' +
							'			descendents.*ancestorId = :id ' +
				...
				.then(function() {
					...
					var sql = 'INSERT INTO ' + schema + '*through (*itemId, *ancestorId) ' +
