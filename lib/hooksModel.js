		...
		beforeUpdate: function(item, options) {
			...
			.then(function(parentLevel) {
				...
				return Promise.try(function() {
					...
					// get the full table name taking schema into account
					var throughTableName = hierarchy.through.getTableName();
					if throughTableName.schema
						throughTableName = throughTableName.toString();
					else
						throughTableName = sequelize.queryInterface.quoteIdentifier(throughTableName);
					...
					var sql = 'UPDATE *item ' +
						...
						'WHERE *id IN (' +
						'	SELECT *itemId ' +
						'	FROM ' + throughTableName + ' AS ancestors ' +
				...
				.then(function() {
					...
					if (['postgres', 'postgres-native'].indexOf(sequelize.options.dialect) != -1) {
						sql = 'DELETE FROM ' + throughTableName + ' ' +
							'USING ' + throughTableName + ' AS descendents, ' + throughTableName + ' AS ancestors ' +
							'WHERE descendents.*itemId = ' + throughTableName + '.*itemId ' +
							'	AND ancestors.*ancestorId = ' + throughTableName + '.*ancestorId ' +
						...
						sql = 'DELETE FROM ' + throughTableName + ' ' +
							'WHERE EXISTS (' +
							'	SELECT * ' +
							'	FROM ' + throughTableName + ' AS deleters ' +
							'		INNER JOIN ' + throughTableName + ' AS descendents ON descendents.*itemId = deleters.*itemId ' +
							'		INNER JOIN ' + throughTableName + ' AS ancestors ON ancestors.*ancestorId = deleters.*ancestorId ' +
							'	WHERE deleters.*itemId = ' + throughTableName + '.*itemId ' +
							'		AND deleters.*ancestorId = ' + throughTableName + '.*ancestorId ' +
							'		AND ancestors.*ancestorId = ' + throughTableName + '.*ancestorId ' +
							'		AND ancestors.*itemId = :id ' +
							'		AND (' +
							'			descendents.*ancestorId = :id ' +
				...
				.then(function() {
					...
					var sql = 'INSERT INTO ' + throughTableName + ' (*itemId, *ancestorId) ' +
