...
module.exports = (Sequelize, patches) => {
	...
	return {
		beforeCreate(item, options) {
			const {primaryKey, foreignKey, levelFieldName} = this.hierarchy,
		...
		beforeUpdate(item, options) {
			...
			const itemId = values[primaryKey],
				...
				oldParentId = item._previousDataValues[foreignKey];

			// If parent not changing, exit - no change to make
			if (parentId === oldParentId || !inFields(foreignKey, options)) return undefined;
			...
			return Promise.try(() => {
				...
				// Check that not trying to make item a child of itself
				if (parentId === itemId) {
					throw new HierarchyError('Parent cannot be a child of itself');
				}
				...
				return findOne(
					...
					if (!parent) throw new HierarchyError('Parent does not exist');
					return parent[levelFieldName];
				});
			}).then((parentLevel) => {
				// Set hierarchy level
				const newLevel = parentLevel + 1,
					// TODO Will not work if level field not populated on input item - FIX!
					levelChange = newLevel - item._previousDataValues[levelFieldName];

				values[levelFieldName] = newLevel;
				addToFields(levelFieldName, options);
				...
				return Promise.try(() => {
					...
					return query(
						sequelize,
						sql,
						addOptions({replacements: {id: itemId, levelChange}}, options)
					);
				}).then(() => {
