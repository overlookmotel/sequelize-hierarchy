...
var utils = module.exports = {
	...
	getModelAttributes: function(model, Sequelize){
		...
		return semverSelect(Sequelize.version || '3.0.0', {
			'>=5.0.0 || >=5.0.0-beta': function() {
				console.log('getModelAttributes: in >=5.0.0 || >=5.0.0-beta ... model.rawAttributes = ' + JSON.stringify(model.rawAttributes));
				return model.rawAttributes;
			},
			'*': function() {
				console.log('getModelAttributes: in * ... model.attributes = ' + JSON.stringify(model.attributes));
	...
	//Cater for breaking changes in new sequelize 5+ versions
	determineUnderscoredAll: function(model, globalOptions){
