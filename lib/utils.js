	...
	replaceTableNames: function(sql, identifiers, sequelize) {
		_.forIn(identifiers, function(model, identifier) {
			...
			if (tableName.schema != null) {
				tableName = sequelize.queryInterface.quoteIdentifier(tableName.schema) + tableName.delimiter + sequelize.queryInterface.quoteIdentifier(tableName.tableName);
			} else {
				tableName = sequelize.queryInterface.quoteIdentifier(tableName);
			}
			sql = sql.replace(new RegExp('\\*' + identifier + '(?![a-zA-Z0-9_])', 'g'), tableName);
		});
		return sql;
	},
