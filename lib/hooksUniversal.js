...
module.exports = {
	afterDefine: function(model) {
		...
		_.forIn(model.attributes, function(field, fieldName) {
			...
			if (!hierarchy.as) {
				if (_.endsWith(fieldName, Utils.uppercaseFirst(primaryKey))) {
...
function convertHierarchy(results, model, parent) {
	...
	_.forIn(references, function(item) {
		...
		if (thisParentId !== parentId) {
			...
			if (!parent) throw new Sequelize.SequelizeHierarchyError('Parent ID ' + thisParentId + ' not found in result set');
