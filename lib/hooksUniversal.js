...
module.exports = function(Sequelize) {
	...
	return {
		afterDefine: function(model) {
			...
			console.log('afterDefine: model.name = ' + model.name);

			// get hierarchy option
			var hierarchy = model.options.hierarchy;

			console.log('afterDefine: hierarchy (1st check) = ' + JSON.stringify(hierarchy));

			var modelAttributes = shUtils.getModelAttributes(model, Sequelize);

			console.log('afterDefine: modelAttributes = ' + JSON.stringify(modelAttributes));

			// check for hierarchy set on a field
			_.forIn(modelAttributes, function(field, fieldName) {
			...
			console.log('afterDefine: hierarchy (2nd check) = ' + JSON.stringify(hierarchy));
			try{
				// if hierarchy set, init hierarchy
				if (hierarchy) model.isHierarchy(hierarchy);
			}
			catch(exc){
				console.log('afterDefine:Exception caught. model = ' + JSON.stringify(model) + '. Exc: ' + exc);
				throw exc;
			}
		},
