...
module.exports = function(Sequelize) {
	...
	return {
		isHierarchy: function(options) {
			...
			var modelAttributes = utils.getModelAttributes(this, Sequelize);
			...
			var underscoredAll = utils.determineUnderscoredAll(this, globalOptions, Sequelize);
			...
			_.defaults(options, {
				foreignKey: patches.underscoredIf(options.as + patches.uppercaseFirst(options.primaryKey), underscored),
				throughKey: patches.underscoredIf(this.name + patches.uppercaseFirst(options.primaryKey), underscored),
				throughForeignKey: patches.underscoredIf(Utils.singularize(options.ancestorsAs) + patches.uppercaseFirst(options.primaryKey), underscored),
				through: this.name + Utils.singularize(options.camelThrough ? patches.uppercaseFirst(options.ancestorsAs) : options.ancestorsAs)
			});
			...
			if (options.freezeTableName) {
				...
				throughTable = patches.underscoredIf(
					...
					(
						...
						patches.uppercaseFirst(options.ancestorsAs) :
						options.ancestorsAs
					), underscoredAll
				);
			}
			...
			modelAttributes[options.levelFieldName] = levelFieldDefinition;
			...
			if (options.foreignKeyAttributes) _.extend(modelAttributes[options.foreignKey], options.foreignKeyAttributes);

			// create labels
			if (options.labels) {
				[options.levelFieldName, options.foreignKey].forEach(function(fieldName) {
					var field = modelAttributes[fieldName];
			...
			var throughFields = {};
			throughFields[options.throughKey] = {type: modelAttributes[options.primaryKey].type, allowNull: false, primaryKey: true};
			throughFields[options.throughForeignKey] = {type: modelAttributes[options.primaryKey].type, allowNull: false, primaryKey: true};
