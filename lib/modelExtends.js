...
module.exports = function(Sequelize, utils) {
	var Utils = Sequelize.Utils,
		_ = Utils._,
		hooks = hooksModel(Sequelize, utils);
	...
	return {
		isHierarchy: function(options) {
			...
			_.defaults(options, {
				...
				through: this.name + Utils.uppercaseFirst(Utils.singularize(options.ancestorsAs)),
				throughTable: Utils.pluralize(this.name) + Utils.uppercaseFirst(options.ancestorsAs)
			});
