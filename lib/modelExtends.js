...
module.exports = function(Sequelize) {
	...
	return {
		isHierarchy: function(options) {
			...
			//If underscored(All) is set to true as global Sequelize option we let it override the model option
			var underscored = this.options.underscored || this.sequelize.options.underscored;
			var underscoredAll = this.options.underscoredAll || this.sequelize.options.underscoredAll;
			...
			var genThroughTableName;
			if (underscoredAll)
				genThroughTableName = patches.underscoredIf(Utils.pluralize(this.name) + Utils.uppercaseFirst(options.ancestorsAs), true);
			else
				genThroughTableName = (Utils.pluralize(this.name) + (options.camelThrough ? Utils.uppercaseFirst(options.ancestorsAs) : options.ancestorsAs));

			_.defaults(options, {
				throughTable: options.freezeTableName ? options.through : genThroughTableName
			});
