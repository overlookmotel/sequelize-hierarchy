...
module.exports = function(Sequelize) {
	...
	return {
		isHierarchy: function(options) {
			...
			var underscoredAll = this.sequelize.options.define.underscoredAll;
			...
			var throughTable;
			if (underscoredAll)
				throughTable = patches.underscoredIf(Utils.pluralize(this.name) + Utils.uppercaseFirst(options.ancestorsAs), true);
			else
				throughTable = (Utils.pluralize(this.name) + (options.camelThrough ? Utils.uppercaseFirst(options.ancestorsAs) : options.ancestorsAs));

			_.defaults(options, {
				throughTable: options.freezeTableName ? options.through : throughTable
			});
