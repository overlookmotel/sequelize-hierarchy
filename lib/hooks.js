...
module.exports = function(Sequelize) {
	...
	return {
		...
		beforeUpdate: function(item) {
			...
			return Promise.resolve().then(function() {
				...
				return hierarchy.through.find({where: where}).then(function(illegalItem) {
			...
			.then(function() {
				...
				return model.find({where: where, attributes: [hierarchy.levelFieldName]}, {raw: true}).then(function(parent) {
		...
		(function traverse(options, model) {
			if (options.include) {
				options.include.forEach(function(include) {
					...
					var accessor = _.uncapitalize(include.as);
	...
	function convertHierarchy(results, model, parent)
	{
		...
		var childsAccessor = _.uncapitalize(hierarchy.childsAs);
		var descendentsAccessor = _.uncapitalize(hierarchy.descendentsAs);
		var throughAccessor = _.uncapitalize(Utils.singularize(hierarchy.through.name));
		
		// get parent id and move results from parent.descendents to parent.childs
		var parentId = null;
		if (parent) {
			parentId = parent[primaryKey];
			
			parent.dataValues[childsAccessor] = parent[childsAccessor] = results;
		...
		_.forIn(references, function(item) {
			...
			if (thisParentId !== parentId) {
				...
				if (!parent[childsAccessor]) parent.dataValues[childsAccessor] = parent[childsAccessor] = [];
				parent[childsAccessor].push(item);
			}
		});
	}
};
